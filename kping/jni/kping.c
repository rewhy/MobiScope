...
static uint64_t lasttsi = 0;
static uint64_t lasttsb = 0;
...
static uint64_t getTimeMicro()
{
	...
	return (uint64_t)ttt.tv_sec * 1000000LL + ttt.tv_usec;
}

static void delay()
{
	uint64_t ddd = (uint64_t)m_cfg->delay * 1000;
	/*if(cfg.cycle > 5000)
	 *	{
	 *			usleep(cfg.cycle-5000);
	 *				}*/
	while (getTimeNano() - lasttsi < ddd)
		;
}
...
static int pkt_sch(unsigned char *pkt, int num, unsigned int len)
{
	...
	psch->work_type= WORK_TYPE_PING;
	psch->len = len;
	psch->num = num;
	psch->tv_delay.tv_sec = 0; //m_cfg->gap.tv_sec;
	...
	lasttsb = getTimeNano();
	res = fsync(tx_fd);
	//printf("KPING(ICMP) %.3f %.3f rtt = %.3f\n", res);
}
...
{
	int pktlen = 0, datalen = sizeof(struct timeval) + 48, i = 0;
	unsigned char *ip_buf = NULL;//kmalloc(pktlen, GFP_KERNEL);
	struct iphdr *iph = NULL;//(struct iphdr *)ip_buf;
	struct icmphdr *icmph = NULL;
	char *data = ip_buf + sizeof(struct iphdr) + sizeof(struct icmphdr);
	...
	icmph = (struct icmphdr *)(ip_buf + sizeof(struct iphdr));
	gettimeofday((struct timeval *)data, NULL);
	icmph->type = ICMP_ECHO;
	icmph->code = 0;
	icmph->checksum = 0;
	icmph->un.echo.id = 0xef;
	icmph->un.echo.sequence = pingIndex++;
	//printf("Debug1\n");
	/*for(i=8; i<48; i++){
		printf("Debug\n");
		data[i] = (char)i;
	...
	//printf("Debug2\n");
	icmph->checksum = csum((void *)icmph, sizeof(struct icmphdr)+datalen);
	//printf("icmp checksum: %d\n", icmph->checksum);
	return pktlen;
}

static void do_ping(int i)
{
	int pktlen = pkt_construct(i);
	//delay();
	//lasttsb = getTimeNano();
...
int ip_process(char *ipbuf, struct timeval tv_rx, int len)
{
	...
	switch(iph->protocol) {
		case IPPROTO_UDP:
			...
			payload = (char *)(udph + 1);
			data = payload;
			if((unsigned char)(*data) != WORK_TYPE_PING)
				return 0;
			data++;
		...
		case IPPROTO_ICMP:
			...
			//printf("KPING %.3f %.3f %.3f ms\n", krtx_inet_ntoa(iph->saddr, buf1), krtx_inet_ntoa(iph->daddr, buf2), ts*1000.0);
			end = getTimeNano();
			printf("KPING %.3f %.3f ms\n", (double)lasttsb / 1000000.0 - (double)lasttsi / 1000000.0, ts*1000.0);
			lasttsi = end;
			//isRx = 0;
			// printf("icmp packet\n");
			break;
...
void thread_rx()
{
	...
	//printf("packet rx wait.\n");
	while(m_cfg->is_rx & rx_fd > 0) {
		/*if(isRx == 0)
			{
		//printf("packet rx wait isRx == %d.\n", isRx);
		usleep(1);
		continue;
		}*/
		do_ping(i);
...
int get_conf(int argc, char *argv[])
{
	...
	while((opt = getopt(argc, argv, "i:d:n:c:h:")) != -1) {
		switch(opt) {
			case 'i':
