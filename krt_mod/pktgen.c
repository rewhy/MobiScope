...
static void pktgen_setup_inject(struct pktgen_dev *pkt_dev)
{
	...
	if (pkt_dev->flags & F_IPV6) {
		...
		if (strlen(pkt_dev->src_min) == 0) {
			...
			// 
			rcu_read_lock(); // 
			// get the receiver device information
...
static void pktgen_run(struct pktgen_thread *t)
{
	...
	list_for_each_entry(pkt_dev, &t->if_list, list) {
		...
		if (pkt_dev->odev) {
			// clear the counters of tx device
			pktgen_clear_counters(pkt_dev);
			pkt_dev->running = 1;	/* Cranke yeself! */
			pkt_dev->skb = NULL;
			// get current timestampin us
			pkt_dev->started_at = getCurUs();
			pkt_dev->next_tx_us = getCurUs();	/* Transmit immediately */
			pkt_dev->next_tx_ns = 0;
			// set pkt overhead for MPLS, VLANs, IPSEC etc.
			set_pkt_overhead(pkt_dev);
...
static int __init pg_init(void)
{
	...
	// Create dir named pktgen under dir proc_net
	pg_proc_dir = proc_mkdir(PG_PROC_DIR, init_net.proc_net);
	if (!pg_proc_dir)
		return -ENODEV;
	pg_proc_dir->owner = THIS_MODULE;

	// Create operation handle pgctrl 
